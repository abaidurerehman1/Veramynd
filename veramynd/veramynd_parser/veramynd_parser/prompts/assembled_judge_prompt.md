...
2. Quote the resource verbatim as proof, with the location. Use a contiguous verbatim span of at least ~24 characters (include the surrounding instructional sentence when the act is a short frame or TDQ such as "At the end …" or "What is the boy doing?"). For any clause you call met, the quote you cite MUST be the student-performed moment (the student_quote) — or, under the Directive/Elicitation Exception, the student-directed imperative — not the teacher's setup.
...
* (Retell/response of a READ story is not narrative creation — 1.T.T.1.e.) Writing or drawing about the events of a story the class has just read (a retell or response sheet) is not "use narrative techniques to CREATE texts" → 1.T.T.1.e none. Creating a POEM (verse planner / "write your own verse") is poetry Construction (1.T.T.4.b), not 1.T.T.1.e — scored against 1.T.T.1.e → none (see F20). Credit summarize (1.P.EICC.3.d) or identify (1.T.T.1.a) on the merits instead. Parenthetical "(e.g., characters, settings, events)" on 1.T.T.1.e is illustrative: do not dock a genuine story-creation task for a missing e.g. item. Q&A about a read story remains 1.T.T.1.a, not 1.T.T.1.e (see the I/C mismatch instance above).
...
* (F13 — select / read / write is separable AND — 1.P.EICC.1.c.) 1.P.EICC.1.c "Select, read, and write texts of personal interest and academic relevance" is three complementary acts. Full requires all three. Teacher-assigned read-aloud / assigned photos = select unmet. Student writing or drawing about that assigned text on a response sheet = write met. Hearing/engaging the assigned grade-level text (R1) can meet read. Write (and optionally read) without student selection → partial — NEVER none when the student write/draw about the text is quotable. Worked instance: teacher assigns Summer Sun Risin'; students write/draw ending events on a response sheet → 1.P.EICC.1.c partial (select absent).
* (F14 — scaffolded (C) frames still count; floor is partial, never none.) When a Construction substandard's named act is performed via a sentence frame, planner, or word bank, the floor is partial — the student performed the act. Do NOT score none merely because the product is framed, pre-filled, or single-use. Full only when independence/scope the standard requires is met. Publisher-agnostic: any curriculum that supplies a transition/sequencing frame the student completes counts. Quote the full instructional sentence that contains the frame (≥24 contiguous characters), not the frame alone. Worked instances: (a) transition/sequencing frame such as "At the end ___," / "Next ___," / "Last ___" on a response sheet → 1.T.SS.1.c (or equivalent) partial — never none; (b) vocab word-choice frame or anchor-chart word bank while speaking/writing → at least partial for use-of-word-relationships standards; (c) independent short writing that yields complete sentences → at least partial for sentence-variety construction standards; (d) craft word-choice for an audience while writing a short piece → at least partial for craft-construction standards.
* (F15 — I/C compounds need BOTH sides for full — 1.P.EICC.1.d / 2.a / 2.c / 2.d / 4.f.) Standards tagged (I/C) that require both interpret and construct (or explain+learn by interpreting AND constructing) are full ONLY when both sides have genuine, substantive student acts. Strong construct alone (write a poem verse) or strong interpret alone (answer questions about a story) → partial, not full. A single quality-check question about a model ("What is something you saw that lets you know the poem is high quality?") while the lesson's weight is independent construction is thin-interpret → keep 1.P.EICC.1.d / 2.a at partial even if construct is strong. 1.P.EICC.2.c: interpret Q&A about the read-aloud PLUS constructing notices/wonders or a response sheet can be full when both are quotable and substantive. 1.P.EICC.2.d still requires analysis/evaluation of ideas (notice/wonder alone remains none — see 2.d instance); peer critique that only checks details/conventions against a writing checklist is craft/quality feedback, not idea-analysis → partial for 2.d (not full). 1.P.EICC.4.f (evaluate effectiveness / match writing goals): a brief peer-protocol pass on details/conventions without a clear student determination that the text matches its purpose/goals → partial. Worked instance: students plan/write verse 3, ask one model-quality question, and run Pinky Partners on details/conventions → 1.P.EICC.1.d / 2.a / 2.d / 4.f partial (not full).
* (F16 — share through a variety of modes — 1.T.RA.1.c.) Oral partner talk PLUS writing/drawing on a response sheet about the same accurate content = a variety of modes → full when the shared information is relevant and accurate. Do NOT dock to partial merely because the modes are brief grade-1 turns. Worked instance: elbow-partner "Who is the main character? (the boy)" plus response-sheet writing/drawing about character and setting → 1.T.RA.1.c full.
* (F17 — refer-to-text needs a student citation act — 1.T.RA.2.a.) A learning target or direction that says "using key details from the text" is NOT enough. Full/partial requires a scripted moment where the student refers to / points to / quotes a specific part of the text to support an idea or answer. Instruction-only with no student citation performance → none. (Contrast R3 Language Dive where the student answer is anchored to a named chunk — that is a real refer act, capped at partial if supplemental-only.)
* (F18 — teacher-supplied meaning + closed practice ≠ context determination — 1.L.V.3.a / 1.P.EICC.3.g.) When the TEACHER states the definition and students only reuse it in a closed prompt ("Where does art class take place?"), the student is not determining meaning from context or word parts → none (not partial). Contrast: student figures meaning from a text chunk ("What does the word by mean in this chunk?") → real context act (R3 caps at partial if supplemental-only).
* (F19 — author's-craft identify/apply without analyze → partial — 1.P.AC.1.a and siblings.) Standards that require identify, apply, AND analyze/explain how elements affect audience or support purpose are separable. Identify + apply literary/expository/opinion elements (e.g. name character and setting; write/draw about them) without analyze/explain-purpose → partial, NEVER none. Analyze-only without identify → partial. Full only when identify/apply AND the analyze/explain-audience-or-purpose act are both student-performed. Publisher-agnostic: same rule for any framework craft standard that stacks identify+apply+analyze. Worked instance: students answer "Who is the main character? / Where does the story take place?" and write/draw about character and setting, but never explain how those elements affect the audience or support purpose → 1.P.AC.1.a partial.
* (F20 — route poem creation to poetry, not narrative-create — 1.T.T.1.e vs 1.T.T.4.b.) Creating a poem (including a narrative poem / verse with planner) is the poetry Construction substandard (1.T.T.4.b), NOT narrative-technique create (1.T.T.1.e). Scored against 1.T.T.1.e → none (wrong text-type), even if the poem has setting/events. Also restates: retell/response of a story the class just read is not 1.T.T.1.e create → none. Publisher-agnostic: match the OUTPUT TYPE family first (story vs poem vs opinion vs informational); a poem product never satisfies a story-create distinguishing clause.
* (F21 — context triad is separable AND; assigned media ≠ research — 1.P.ST.1.a.) 1.P.ST.1.a requires three independent clauses: (1) use prior knowledge, (2) use formal or informal research, (3) engage in discussions — all toward identifying key components of context. Full only when all three are student-performed. Prior knowledge + discussion without a genuine research act → partial (never full). Assigned classroom media (teacher-selected read-aloud, photos, or videos students watch/listen to) is content delivery, NOT informal research. Informal research requires a student investigation/gathering act (look up, inquire, gather information as research — e.g. student-chosen sources, research questions they pursue, or an explicit research task). Publisher-agnostic: consuming the lesson's provided text/media ≠ research. Worked instance: title prediction + Picture Tea Party / Back-to-Back discussion + watching sun/moon/stars videos → 1.P.ST.1.a partial (research unmet).
...
* Do not treat 1.P.EICC.2.b's four purposes as separable (any one student-performed purpose = full). Do not treat 1.T.T.1.a's four story techniques as interchangeable (all four required for full). Do not credit notice/wonder as 1.P.EICC.2.d analysis. Do not credit a retell/response of a read story as 1.T.T.1.e creation. Do not treat 1.P.CP.1.d's collaborate-to-discuss/investigate/solve/create list as separable (any one student collaborative act = full — F9). On 1.L.V.1.a, acquire AND apply are both required and vocab buckets are OR; on 1.L.V.1.b, "variety of settings" is a per-lesson ≥2-settings cap. Do not dock for missing parenthetical e.g. items (F5). Do not score ambient grammar as Partial (F6). Do not apply 1.T.T.2.* to narrative text (F7). Apply R3: supplemental-only student acts cap at partial (not none, not full). Do not credit 1.P.CP.2.d dialogue-ask from written wonders or learning-target wording alone. On 1.P.EICC.4.c, score the generate-ideas product fed by the named channels (F10), not three isolated checklists. On identify+explain (F11) and describe+message (F12), missing the second operation → partial. On 1.P.EICC.1.c, write-without-select is partial not none (F13). Scaffolded (C) frames floor at partial, never none (F14). I/C compounds need both sides for full (F15). Oral+written share can be full for 1.T.RA.1.c (F16). "Using key details" wording alone is not 1.T.RA.2.a (F17). Teacher-supplied closed vocab practice is none for context-determination (F18). Craft identify+apply without analyze → partial (F19). Poem products score as poetry-create, not narrative-create 1.T.T.1.e (F20). On 1.P.ST.1.a, prior+discussion without a genuine student research act is partial — assigned read-aloud/videos ≠ informal research (F21).
