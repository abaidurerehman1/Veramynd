...
Every downstream stage (normalize, embed, retrieve, judge) must refuse
...
def warn_untrusted_input_file(path: str | None, kind: str) -> int | None:
    ...
    ``verification_verdict.json``. Ad-hoc files with no provenance artifact
    are allowed (debugging is legitimate); a provenance artifact that says
    BLOCK/unverified is a hard error. Returns an exit code on failure, else None.
    """
    if not path or kind != "lesson":
        return None
    ...
    # (…/lessons/X.json). A bare relative name would otherwise walk up to cwd
    # lexically and match whatever provenance file happens to live there.
    p = Path(path).resolve()
    if p.parent.name != "lessons":
        return None
