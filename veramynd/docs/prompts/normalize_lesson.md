...
3. **Evidence is mandatory.** Every non-`NONE OBSERVED` student action must be
   ...
   exactly, but strip PDF furniture that is not part of the sentence — running
   headers/footers, standalone page numbers (e.g. a bare `74`), grade/module
   banners, and mid-word hyphenation from line breaks. The quote must be the
   clean instructional text so the engine can string-match it.
...
**Do not assert an omission that the lesson's own steps contradict** — if any
step or feedback example shows the skill happening, it is not "not taught."
